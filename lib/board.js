...
Serial = {
  ...
  detect: function(callback) {
    ...
    serialport.list(function(err, result) {
      ...
      if (!length) {
        ...
        this.emit("error", { message: "No USB devices detected" });
  ...
  connect: function(usb, callback) {
    ...
    try {
      io = new Firmata(usb, function(error) {
        if (error !== undefined) {
          err = error;
        }
...
function Board(opts) {
  ...
  if (this.mock) {
    this.ready = true;
    this.io = new Firmata(this.mock, function() {});
    ...
    // TODO: Refactor all of this to allow a PROVIDED io layer
    // access to the same output and process event bindings
    //
    //
    Serial.detect.call(this, function(port) {
      Serial.connect.call(this, port, function(err, type, io) {
        if (err) {
